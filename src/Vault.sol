...
contract Vault is IERC721Receiver {
    ...
    function fractionalize(address nftAddress, uint256 nftId, uint256 supply, string memory name, string memory symbol)
        public
    {
    ...
    function withdrawNftNotFractionalized(address _nft, uint256 _id) public {
        uint256 _index = index[_nft][_id];
        require(
            AccessDeposits[msg.sender].deposits[_index].Fractionalised == false
                && AccessDeposits[msg.sender].deposits[_index].owner == msg.sender
        );
